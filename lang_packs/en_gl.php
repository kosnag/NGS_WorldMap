<?
$localization = array (
	'UI' => array(
		'title' => "PSO2NGS Interactive Map",
		'languages' => array(
			'switcher' => "Language",
			'en_gl' => "English (Global)",
			'en_al' => "English (Arks-Layer)",
			'jp' => "Japanese",
			'ru' => "Russian",
			'kr' => "Korean",
		),
		'sorting' => array(
			'title' => "Sorting",
			'1' => "Main objects",
			'2' => "Boxes and Reactors",
			'3' => "Minerals",
			'4' => "Vegetables and etc",
			'5' => "Bosses",
		),
	),
	'objects' => array(
		'centtower' => "Central Tower (Aelio)",
		'cocoon' => array(
			'1' => "Enhanced Enemy",
			'2' => "Swift Jump",
			'3' => "<b>Cocoon</b> (1 Player)<br><h1>First Steps</h1><p>This is the facility where an Instructor who tests the basic abilities of every ARKS.<br>By following the instructions, one can learn new abilities while reaching the targeted goal.</p>",
			'4' => "Wild Rush",
			'5' => "Great Wall",
			'6' => "Cocoon ID-6",
			'7' => "Cocoon ID-7",
		),
		'mag' => array(
			'1' => "Expert Region Mag",
		),
		'ryuker' => array(
			'1' => "Ryuker Device ID-1",
			'2' => "Ryuker Device ID-2",
			'3' => "Ryuker Device ID-3",
			'4' => "Ryuker Device ID-4",
			'5' => "Ryuker Device ID-5",
			'6' => "Ryuker Device ID-6",
			'7' => "Ryuker Device ID-7",
			'8' => "Ryuker Device ID-8",
			'9' => "Ryuker Device ID-9",
			'10' => "Ryuker Device ID-10",
			'11' => "Ryuker Device ID-11",
			'12' => "Ryuker Device ID-12",
			'13' => "Ryuker Device ID-13",
			'14' => "Ryuker Device ID-14",
		),
		'tower' => array(
			'1' => "Tower ID-1",
			'2' => "Tower ID-2",
		),
		'city' => array(
			'1' => "City ID-1",
			'2' => "City ID-2",
		),
		'hut' => array(
			'1' => "Hut ID-1",
			'2' => "Hut ID-2",
			'3' => "Hut ID-3",
		),
		'urgent' => array(
			'1' => "Urgent Quest Location ID-1",
		),
		'dungeon' => array(
			'1' => "Dungeon ID-1",
		),
	),
	'BaR' => array(
		'boxes' => array(
			'red' => "Item Container: Red",
			'white' => "Item Container: White",
			'gold' => "Item Container: Gold",
		),
		'alphareactor' => "Aplha Reactor",
		'datapod' => "Datapod",
	),
);
